...
public abstract class DataListItemLocator<T> implements IOverviewItemLocator<T> {
	...
	protected abstract Class<? extends PartView<?>> getPartViewClass();
	...
	private String showView() {
		...
		final String partId = partStack.getElementId() + "." + getPartName();
	...
	protected String getPartName() {
		return getToolItemId().substring(Activator.MAIN_TOOLBAR_ID.length() + "item.".length() + 1);
	}
}
