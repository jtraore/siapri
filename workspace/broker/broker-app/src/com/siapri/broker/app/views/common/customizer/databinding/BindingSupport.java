...
public class BindingSupport {
	...
	@SuppressWarnings("unchecked")
	public void bindCheckBox(final Object object, final String property, final Widget widget) {
		final IObservableValue<?> widgetValue = WidgetProperties.selection().observe(widget);
		final IObservableValue<?> modelValue = BeanProperties.value(property).observe(object);
		bindingContext.bindValue(widgetValue, modelValue);
	}
