...
public class InsuranceTypeCustomizer extends AbstractCustomizer<InsuranceType> {
	...
	public Composite createArea(final Composite parent, final int style) {
		...
		final WarrantyDataListModel warranyDataListModel = new WarrantyDataListModel(composite, object);
		warranyDataListModel.setFilterDisplayed(false);
		warranyDataListModel.setReportButtonDisplayed(false);
		final DataListComposite warrantyDataListComposite = new DataListComposite(composite, SWT.NONE, warranyDataListModel);
		final GridData warrantiGridData = new GridData(GridData.FILL, GridData.FILL, true, true, 6, 1);
		warrantiGridData.heightHint = 300;
		warrantyDataListComposite.setLayoutData(warrantiGridData);
		
		new TitledSeparator(composite, "Liste des formules").setLayoutData(new GridData(SWT.FILL, SWT.DEFAULT, true, false, 6, 1));

		final FormulaDataListModel formulaDataListModel = new FormulaDataListModel(composite, object);
		formulaDataListModel.setFilterDisplayed(false);
		formulaDataListModel.setReportButtonDisplayed(false);
		final DataListComposite formulaDataListComposite = new DataListComposite(composite, SWT.NONE, formulaDataListModel);
		final GridData formulaGridData = new GridData(GridData.FILL, GridData.FILL, true, true, 6, 1);
