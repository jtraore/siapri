package com.siapri.broker.app.views.preferences;
...
public class AppPreferenceStore implements IPreferenceStore {

	private final Map<String, Preference> preferences = new HashMap<>();
	private final IBasicDaoService daoService = BundleUtil.getService(IBasicDaoService.class);

	public AppPreferenceStore() {
		final List<Preference> preferenceList = daoService.getAll(Preference.class);
	...
	public void saveAll() {
		daoService.saveAll(new ArrayList<>(preferences.values()));
	}

	public void save(final String preferenceKey) {
		if (preferences.containsKey(preferenceKey)) {
			daoService.save(preferences.get(preferenceKey));
		}
	}
}
